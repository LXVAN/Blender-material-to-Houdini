        ...
        for material in obj.data.materials:                    
            ...
            for link in links:
                ...
                if fromNode.type == 'TEX_IMAGE':
                    ...
                    imageName = fromNode.image.name
                    
                    

                    if (toNode.type =="NORMAL_MAP") | (toNode.type == "OUTPUT_MATERIAL") | (toNode.type == "BUMP") | (toNode.type =="DISPLACEMENT"):
                        #检车图像材质连接的toNode 是否为 normal, 材质displacement,displacement,bump
                        if (toNode.type == "OUTPUT_MATERIAL")|(toNode.type =="DISPLACEMENT"):
                            toSocketName = "trueDisplacement"#连接到材质输出上的置换应该是"真"置换
                        else:
                            toSocketName = toNode.type
                    

                    #pbrList.append(f'{toSocketName}:{imagePath}')
                    toSocketNameList.append(f"{toSocketName}")#贴图种类的列表
                    imagePathList.append(f"{imagePath}")#路径的列表
                    #pbrList.append(obj.name +':'+material.name + ':' + f'{toSocketName}:{imagePath}'+'\n')                                        
                    #materialInformation0 ={idx + 1: name for idx, name in enumerate(pbrList)}#会有数字输出,不好读取
                    materialInformation0 = dict(zip(toSocketNameList,imagePathList))#将贴图和路径的列表拼起来,一一对应
...
NUM = {
    "Success" :f"{SuccessNum}",
    "Fail" :f"{MatNum }"
}
#print (NUM)
        
jsonFile = os.path.dirname(os.path.abspath(__file__)) + "\\BlenderMaterial.json"
#f"Success:{SuccessNum}\n"+f"Fail:{MatNum-SuccessNum}\n"
...
'''
5.23预计完成
把遍历所有物体去掉,也就是只用找到所有材质
把法线(法向),置换,凹凸之类的做一下判断
把输出的json文件再让houdini py读取,生成matnet和各个principled shader
'''
